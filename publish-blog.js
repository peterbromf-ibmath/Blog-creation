...
async function generateArticle(apiKey, topic) { const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${encodeURIComponent(apiKey)}`; const prompt = `You are Pete Bromfield, an IB Maths teacher. Write a punchy blog post (600 words) on: ${topic.label}. Return valid JSON: { "title": "...", "subtitle": "...", "html": "...", "socialCaption": "...", "callToAction": "${topic.cta}" }`; const body = { contents: [{ role: "user", parts: [{ text: prompt }] }] }; const maxAttempts = 4; for (let attempt = 1; attempt <= maxAttempts; attempt++) { try { const res = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }); const json = await res.json(); if (!res.ok) { const isRetryable = res.status === 503 || res.status === 429; console.warn(`[publish-blog] Gemini attempt ${attempt} failed: HTTP ${res.status} — ${JSON.stringify(json).slice(0, 200)}`); if (isRetryable && attempt < maxAttempts) { const waitMs = attempt * 15000; // 15s, 30s, 45s console.log(`[publish-blog] Retrying in ${waitMs / 1000}s...`); await new Promise(r => setTimeout(r, waitMs)); continue; } throw new Error(`Gemini API error ${res.status}: ${JSON.stringify(json)}`); } const text = json?.candidates?.[0]?.content?.parts?.[0]?.text || ""; if (!text) throw new Error("Gemini returned an empty response."); const s = text.indexOf("{"); const e = text.lastIndexOf("}"); return JSON.parse(text.slice(s, e + 1)); } catch (e) { if (attempt === maxAttempts) throw e; console.warn(`[publish-blog] Gemini attempt ${attempt} failed: ${e.message}`); await new Promise(r => setTimeout(r, attempt * 15000)); } } } 
